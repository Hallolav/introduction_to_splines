...
###########################
source('load_data.R')
# put your packages here - if you use them. It's best to do everything by hand,
# so that you do not have to rely on somebody else's work. However, this is of
...
shift <- function(x){
  ...
  stop('This part of the code is not implemented yet. Please do assignment 1')
...
shift_back <- function(min_x = min_x, max_x = max_x, x = x){
  # this function reverts back the min-max scaling from before. Please finish
  # this function
  
  stop('This part of the code is not implemented yet. Please do assignment 2')
...
add_piecewise_basis <- function(degree, x){
  ...
  stop('This part of the code is not implemented yet. Please do assignment 3')
...
find_beta <- function(x, y){
  ...
  stop('This part of the code is not implemented yet. Please do assignment 4')
...
make_prediction = function(x, beta)  {
  ...
  stop('This part of the code is not implemented yet. Please do assignment 5')
...
my_piecewise_polynomial <- function(x, y, knots, degree = 3, ylim = NULL){
  ...
  for (knotindex in 0:length(knots)){
    ...
    prediction <- make_prediction(x = x_test, beta = beta, years = datasubset_test[,1])
...
#           splines           #
###############################
check_spline_conditions <- function(x, y, knots, degree){
  # this function checks some basic conditions required in order to do
  # regression. You don't have to complete this function.
  # check x/y conditions
  if (length(x) != length(y)){
  ...
  # check degree conditions
  if (degree%%1 != 0){
    msg = sprintf('Degree %.3f not integer. Please select degree 2, 3, 4, 5 or 6', degree)
    return(c(T, msg))
  }
  
  if(degree <= 1){
    msg = sprintf('Degree %i too low. Please select degree at least 1', degree)
    return(c(T, msg))
  } 
  
  if (degree >4){
    msg = sprintf('Degree %i too high. Please select degree at most 6', degree)
    return(c(T, msg))
  }
...
my_spline <- function(x, y, knots, degree = 3){
  stop_value = check_spline_conditions(x, y, knots, degree)
  if (stop_value[1]){
    stop(stop_value[2])
  }
...
add_natural_spline_basis <- function(degree, years, knots){
  ...
  for (knot in knotsubset){
    ...
    stop('This part of the code has not been implemented yet. Finish the lines
         below. Please do assignment 9. Which numbers should be filled in the
         function d() below?')
